...
const serviceConfig = [
  { key: 'import', slug: 'import', Icon: Import },
  { key: 'export', slug: 'export', Icon: ArrowUpFromLine },
  { key: 'logistik', slug: 'logistik-zoll', Icon: Truck },
] as const;
...
export default async function LeistungenPage({ params }: Props) {
  ...
  return (
    <section className="max-w-4xl mx-auto px-4 py-16 md:py-24">
      ...
      {/* Service Cards — single column for detail */}
      <div className="flex flex-col gap-8">
        {serviceConfig.map(({ key, slug, Icon }) => {
          const features = t.raw(`items.${key}.features`) as string[];

          return (
            <div
              key={key}
              className="bg-surface border border-mist rounded-xl p-8"
            >
              {/* Service Icon + Title */}
